...
- Desconto: REMOVIDO da calculadora por completo (Victor 17/06/2026). O vendedor envia o valor cheio; o desconto é decidido no próprio GestãoClick. Sem campo de desconto, sem limite por perfil, sem modal de gerente; a tela admin de "Configurações" também foi removida. **Limpeza de vestígios em 30/06/2026 (migração `20260630170000_cleanup_vestigios`): removidas as colunas `usuarios.desconto_max_pct`, `orcamentos.desconto_pct`, `orcamentos.desconto_aprovado_por` (+ relação aprovador) e a tabela `itens_orcamento` (nunca usada; os itens vivem em `orcamentos.itens_json`).** A tabela `configuracoes` foi MANTIDA — não é vestígio: hoje guarda as Regras de Cálculo (chave `regras_calculo`).
...
- **Preço da PERSIANA = soma de TODOS os componentes + tecido, a VAREJO (26/06/2026 — substitui a RN-03 "só tecido por m²").** Cada preço é puxado do GestãoClick pelo `codigo_interno` (grupos 190128 + 76945). Receitas das planilhas do Victor em `services/calc/persianaReceitas.data.ts`; motor puro em `services/calc/persianaPreco.ts`; ponte com o GC em `services/calc/persianaPrecoGc.ts`; quantidades avaliadas por `formula.ts → evalQuantidade` (respeita parênteses; vars LARGURA/ALTURA/TC e, na romana, CAVALETES/HASTES). Cobre as 7 persianas (manual + motor; romana sem motor; tela solar = tecido por m²). O motor antigo `services/calc/persiana.ts` (m²) ficou só para testes/composição, **fora do caminho de preço**. A tela mostra "Ver componentes" por item. Detalhes e regras confirmadas pelo Victor em decisions.md §19–§21.
...
- ~~BLOQUEANTE-05: Fator de tecido do WAVE = 2,7~~ CONFIRMADO em 23/06/2026 ("Acredito que 2,7 vai dar certo"). Mantém fator fixo 2,7 (wave NÃO respeita franzido). Já implementado (`reg.franzido_wave`). EM ABERTO (não respondidos — levar à call): quantidade da **fita wave** + se **Terminais** saem nos outros modelos de trilho (hoje só no wave, qtd 4).
- ~~BLOQUEANTE-07: "Cortina simples + adicionar tecido"~~ RESOLVIDO em 24/06/2026 (v.4.1). Cada CAMADA tem MODELO próprio (frente wave + fundo franzido); entretela por camada; nome do produto = somatória ("AMBIENTE, Cortina MODELO1 TEC1 + MODELO2 TEC2 LxA"). Fixação é única da cortina (interseção das fixações dos modelos). EM ABERTO: fixação DIFERENTE por camada (confirmar com Victor se precisa).
- EM ABERTO (v.4.1, dependem do Victor): (A) franzido > 3× "joga o cálculo muito acima" na cortina COM EMENDA — precisa do caso exato (medidas+franzido+valor esperado); ~~(C) persiana: componentes "têm custo"~~ **RESOLVIDO em 26/06/2026** — persiana agora soma todos os componentes + tecido (ver Decisões Registradas e decisions.md §19–§21); fixação por camada (ver BLOQUEANTE-07).
