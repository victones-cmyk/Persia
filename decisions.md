...
## 17. Retorno da homologação v.4.1 (24/06/2026)

Victor aprovou quase tudo do roteiro v.4 (instalação por peça, emenda c/ franzido, suporte 0, nome "Cortina/Persiana", orçamento misto, seletor "o que incluir", tooltip de composição). Pontos com ação:

- **B — Tooltip mostrava códigos do DecorSoft (não do GC):** confirmado (os `materialCodigo` de `componentes.data.ts` vêm da planilha DecorSoft). Corrigido: a `composicao.ts` **não exibe mais código** nos componentes da persiana (que, além disso, não entram no preço). Cortina segue com os grupos reais do GC.
- **D — Nome do produto com o AMBIENTE na frente:** persiana → "AMBIENTE, Persiana …" (`orcamentoController.nomeProdutoGc`); cortina → "AMBIENTE, Cortina MODELO1 TEC1 + MODELO2 TEC2 LxA" (`orcamentoCortinaController`, somatória das camadas; trunca em 100 chars). 
- **F — Wave:** (F1) itens obrigatórios (Cordão/Rodízio wave/Base click/**Fita wave**) **resolvidos pelo servidor** — vendedor não escolhe (grupo WAVE tem 1 produto por tipo; `ehWaveFixo`/`resolverProdutoWaveFixo` em `acessorios.ts`); só **Terminais** é selecionável (3 opções). (F2) **Terminais** agora saem em **todo trilho/varão suíço** (qualquer modelo; trilho conta 1×, varão suíço soma por camada). (F3) **Fita wave** qty = largura franzida ("franzido em cima"): `frente.metragem` (sem emenda) / `consumoFrente` (com emenda).
- **E — Cortina "simples + adicionar tecido":** cada **camada tem MODELO próprio** (Victor v.4.1: frente wave + fundo franzido) — `CamadaCortina.modelo`, motor roda cada camada com seu modelo. **Entretela por camada** (cada camada cujo modelo use entretela; não mais "só na frente"). UI `CortinaCard` reescrita: modelo por camada, fixação única (interseção das fixações dos modelos), itens wave sem seletor. **Fixação por camada NÃO** foi feita (uma fixação por cortina cobre "frente wave + fundo franzido no mesmo trilho/varão suíço") — confirmar com Victor se precisa de fixação DIFERENTE por camada.
- **Verificado:** 71 testes (cortina.test atualizado: entretela por camada + caso frente wave+fundo franzido); typecheck API + build web; `prepararCortina` e2e via tsx (nome somatório, wave auto-resolvido, fita wave 8,1, terminais no trilho, entretela só da camada wave). Falta só o check VISUAL da tela de cortina (precisa de login).

### Em aberto (precisam do Victor — fora desta implementação)
